...
import time
from internal.yolov7_pose.models.experimental import attempt_load
from internal.yolov7_pose.utils.datasets import LoadStreams, LoadImages, letterbox
from internal.yolov7_pose.utils.general import check_img_size, non_max_suppression, scale_coords
from internal.yolov7_pose.models.yolo import Model
from internal.yolov7_pose.utils.plots import plot_skeleton_kpts
import time
...
class Y7Detect:
    def __init__(self, weights='yolov7_pose/weights/yolov7-w6-pose.pt'):
        """
...
def draw_kpts(image, kpts):
    for kpt in kpts:
        ...
        cv2.polylines(image, [kpt[line_head]], False, (0, 255, 20), 5)
...
if __name__ == '__main__':
    path_models = './weights/yolov7-w6-pose.pt'

    url = '/home/tungpt37/Workspace/frame_00001.png'
    img = cv2.imread(url)
    y7_model = Y7Detect(weights=path_models)
    bboxes, labels, scores, lables_id, kpts = y7_model.predict(img)
    print(np.asarray(bboxes).shape)
    print(np.asarray(scores).shape)
    print(np.asarray(kpts).shape)
    print(kpts)
